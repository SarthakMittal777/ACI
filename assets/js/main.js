...
(function ($) {
	...
	const aci_all_btns = gsap.utils.toArray(".btn_wrapper");
	if (aci_all_btns.length > 0) {
		var aci_all_btn = gsap.utils.toArray(".btn_wrapper");
	}
	else {
		var aci_all_btn = gsap.utils.toArray("#btn_wrapper");
	}
	const aci_all_btn_cirlce = gsap.utils.toArray(".btn-moving");
	aci_all_btn.forEach((btn, i) => {
		$(btn).mousemove(function (e) {
			callParallax(e);
		});
		function callParallax(e) {
			parallaxIt(e, aci_all_btn_cirlce[i], 80);
		}
		...
		$(btn).mouseleave(function (e) {
			gsap.to(aci_all_btn_cirlce[i], 0.5, {
				x: 0,
				y: 0,
				ease: Power2.easeOut,
			});
